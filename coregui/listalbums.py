...
class AlbumSelect:
    ...
    def __init__(self, root, entrybox, entries):
        ...
        logging.info("AlbumSelect.__init__()...")
        ...
        logging.info("AlbumSelect: creating ListBox with albums...")
        ...
        logging.info("AlbumSelect: returning...")
...
def getAlbums(email):
    ...
    logging.info("getAlbums(): email: [%s], client secrets=[%s]"%(email, client_secrets))

    credential_store = os.path.join(configdir, 'credentials.dat')
    
    logging.info("getAlbums(): credential_store: " + credential_store)

    gd_client = OAuth2Login(client_secrets, credential_store, email)

    logging.info("getAlbums(): back from OAuth2Login()")

    albums = gd_client.GetUserFeed()

    logging.info("getAlbums(): back from getUserFeed()")
    entries = []
